...
// from other codecs to the internally used matroska ones
static const struct StereoModeConversionMap CodecModeToVideoModeMap[] =
{
  // asf / wmv
    ...
    case AVMEDIA_TYPE_VIDEO:
      {
        ...
        std::string stereoMode = GetStereoModeFromStream(pStream);
...
std::string CDVDDemuxFFmpeg::GetStereoModeFromStream(const AVStream* pStream)
{
  return GetStereoModeFromMetadata(pStream->metadata);
  //@todo add some technical fallback detection methods, like checking for the black pixel gaps in SBS/OU pictures?
}
...
std::string CDVDDemuxFFmpeg::GetStereoModeFromMetadata(AVDictionary* pMetadata)
{
  ...
  if (stereoMode.empty())
  {
    ...
    if (tag && tag->value)
    {
      ...
      if (tag && tag->value)
        stereoMode = ConvertCodecToInternalStereoMode(tag->value);
    }
  }
...
{
  size_t i = 0;
  while (CodecModeToVideoModeMap[i].name)
  {
    if (mode == CodecModeToVideoModeMap[i].name)
      return CodecModeToVideoModeMap[i].mode;
    i++;
  }
  return "";
}
